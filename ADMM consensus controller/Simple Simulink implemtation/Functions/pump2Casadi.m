...
%% Water level in water tower (need for the cost functions)
    h= c.g0*c.rhoW*1/c.At*(c.A_2*(c.A_1*u(1:total,1)-c.d)+c.V);
        ...
        J_l = ones(1,c.Nc)*(c.e2*c.Je/1000.*(c.A_32*(u(1:total,1).*u(1:total,1).*abs(u(1:total,1))/3600^3*c.rf2 + u(1:total,1)/3600*c.g0*c.rhoW*c.z2)+(c.A_32*u(1:total,1)/3600).*h+c.A_32*u(1:total,1)/3600.*(c.rfTogether*(c.A_1*abs(u(1:total,1))/3600).*(c.A_1*u(1:total,1)/3600))));
